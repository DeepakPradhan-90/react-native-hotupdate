...
public class FileUtils {
    ...
    public static void deleteDirectoryAtPath(String directoryPath) {
        if (directoryPath == null) {
            Log.wtf("FileUtils","deleteDirectoryAtPath attempted with null directoryPath");
            return;
        }
    ...
    public static void deleteFileOrFolderSilently(File file) {
        ...
        if (!file.delete()) {
            Log.wtf("FileUtils","Error deleting file " + file.getName());
        }
    }
    ...
    public static String getSHA256HashFromFile(String filename) {
        return getSHA256HashFromFile(new File(filename));
    }

    public static String getSHA256HashFromFile(File file) {
        if (!file.isFile()) {
            return null;
        }
        try {
            HashCode code = asByteSource(file).hash(Hashing.sha256());
